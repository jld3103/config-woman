...
def test_files():
    with open('system.yaml', 'w') as file:
        file.write(yaml.dump({'files': {'/etc/test.txt': '1:2:611'}}))
    os.makedirs('system-files/etc')
    with open('system-files/etc/test.txt', 'w') as file:
        file.write('bla')
    subprocess.run(['python', 'main.py', 'system', 'apply'])
    with open('system-files/etc/test.txt', 'r') as original_file:
